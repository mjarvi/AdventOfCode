import unittest
import math
from inputs import DAY_1_INPUT
...
def get_cumulative_fuel_amount(mass):
    m = get_fuel_amount(mass)
    return m + get_cumulative_fuel_amount(m) if m > 0 else 0


class TestDayOne(unittest.TestCase):

    def test_examples_I(self):
        self.assertEqual(2, get_fuel_amount(12))
        self.assertEqual(2, get_fuel_amount(14))
        self.assertEqual(654, get_fuel_amount(1969))
        self.assertEqual(33583, get_fuel_amount(100756))

    def test_I_solution(self):
        self.assertEqual(
            3188480,
            sum(get_fuel_amount(int(m)) for m in DAY_1_INPUT.splitlines()))

    def test_examples_II(self):
        self.assertEqual(2, get_cumulative_fuel_amount(14))
        self.assertEqual(966, get_cumulative_fuel_amount(1969))
        self.assertEqual(50346, get_cumulative_fuel_amount(100756))

    def test_solution_II(self):
        self.assertEqual(
            4779847,
            sum(get_cumulative_fuel_amount(int(m)) for m in DAY_1_INPUT.splitlines()))
