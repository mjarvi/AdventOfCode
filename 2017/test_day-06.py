...
blocks are spread out over the memory banks. The fourth, first, and second
banks get two blocks each, and the third bank gets one back. The final result
looks like this: 2 4 1 2.
Next, the second bank is chosen because it contains the most blocks (four).
Because there are four memory banks, each gets one block. The result is: 3 1 2
3.  Now, there is a tie between the first and fourth memory banks, both of
...
of the four banks receives one: 1 3 4 1.  The third bank is chosen, and the
same thing happens: 2 4 1 2.
...
example = [0, 2, 7, 0]
problem = [11, 11, 13, 7, 0, 15, 5, 5, 4, 4, 1, 1, 7, 1, 15, 11]


def get_loop_count_to_reach_balance(banks):
    ...
    while banks not in history:
        ...
        banks = balance(banks)
        loops += 1
    last_time_in_history = len(history) - history.index(banks)
    return loops, last_time_in_history
