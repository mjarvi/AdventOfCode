...
def is_valid_I(string):
    occurrences = sorted(Counter(string.split()).values())
    return False if occurrences[-1] > 1 else True


def is_valid_II(string):
    def anagrams(strgin):
        for i, a in enumerate(string.split()):
            for j, b in enumerate(string.split()):
                if i == j:
                    continue
                if sorted(a) == sorted(b):
                    return True
        return False
    return is_valid_I(string) and not anagrams(string)


class TestDayFour(unittest.TestCase):

    def test_examples_I(self):
        self.assertEqual(True, is_valid_I('aa bb cc dd ee'))
        self.assertEqual(False, is_valid_I('aa bb cc dd aa'))
        self.assertEqual(True, is_valid_I('aa bb cc dd aaa'))

    def test_solution_I(self):
        valid_pass_phrases = [p for p in phrases.splitlines() if is_valid_I(p)]
        self.assertEqual(383, len(valid_pass_phrases))

    def test_examples_II(self):
        self.assertEqual(True, is_valid_II('abcde fghij'))
        self.assertEqual(False, is_valid_II('abcde xyz ecdab'))
        self.assertEqual(True, is_valid_II('a ab abc abd abf abj'))
        self.assertEqual(True, is_valid_II('iiii oiii ooii oooi oooo'))
        self.assertEqual(False, is_valid_II('oiii ioii iioi iiio'))

    def test_solution_II(self):
        valid_pass_phrases = [p for p in phrases.splitlines() if is_valid_II(p)]
        self.assertEqual(265, len(valid_pass_phrases))
