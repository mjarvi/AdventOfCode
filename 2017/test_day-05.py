...
def get_jumps(numbers, increment):
    maze = [int(n) for n in numbers.splitlines()]
    steps = 0
    current = 0
    while 0 <= current < len(maze):
        next_position = maze[current]
        maze[current] += increment(next_position)
        current += next_position
        steps += 1
    return steps


def complicated_jump(length):
    return -1 if length >= 3 else +1


class TestDayFive(unittest.TestCase):

    def test_examples_I(self):
        self.assertEqual(5, get_jumps(example, lambda jump: +1))

    def test_solution_I(self):
        self.assertEqual(325922, get_jumps(problem, lambda jump: +1))

    def test_examples_II(self):
        self.assertEqual(10, get_jumps(example, complicated_jump))

    def test_solution_II(self):
        self.assertEqual(24490906, get_jumps(problem, complicated_jump))
